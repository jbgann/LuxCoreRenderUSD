...
//
#ifndef HDLUXCORE_SAMPLER_H
#define HDLUXCORE_SAMPLER_H
...
// Define template specializations of HdLuxCoreTypeHelper methods for
// all our supported types...
#define TYPE_HELPER(T,type)\
template<> inline HdTupleType \
HdLuxCoreTypeHelper::GetTupleType<T>() { return HdTupleType{type, 1}; }
...
public:
    ...
    HdLuxCoreBufferSampler(HdVtBufferSource const& buffer)
        : _buffer(buffer) {}
    ...
    template<typename T> bool Sample(int index, T* value) const {
        return Sample(index, static_cast<void*>(value),
            HdLuxCoreTypeHelper::GetTupleType<T>());
    }
...
public:
    /// Default constructor.
    HdLuxCorePrimvarSampler() = default;
    /// Default destructor.
    virtual ~HdLuxCorePrimvarSampler() = default;
