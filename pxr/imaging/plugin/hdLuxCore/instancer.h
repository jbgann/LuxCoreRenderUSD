...
//
#ifndef HDLUXCORE_INSTANCER_H
#define HDLUXCORE_INSTANCER_H
...
/// \class HdLuxCoreInstancer
///
/// HdLuxCore implements instancing by adding prototype geometry to the BVH
/// multiple times within HdLuxCoreMesh::Sync(). The only instance-varying
/// attribute that HdLuxCore supports is transform, so the natural
...
///
class HdLuxCoreInstancer : public HdInstancer {
public:
    /// Constructor.
    ...
    HdLuxCoreInstancer(HdSceneDelegate* delegate, SdfPath const& id,
...
private:
    ...
    // by HdLuxCoreMesh::Sync(), which is dispatched in parallel, so it needs
